  ...
  return (
    <tr className="hover:bg-blue-50/40 transition-all duration-150 border-b">
      ...
      <td className="px-3 sm:px-4 md:px-6 lg:px-8 py-4 md:py-5 lg:py-6">
        <p className="font-semibold text-gray-900 text-xs sm:text-sm truncate max-w-[120px]" title={order?._id}>
          {order?._id}
        </p>
      </td>
      ...
      {/* User */}
      <td className="px-3 sm:px-4 md:px-6 lg:px-8 py-4 md:py-5 lg:py-6">
        <p className="text-gray-700 text-xs sm:text-sm truncate max-w-[120px]" title={order?.userId?.name}>
          {order?.userId?.name}
        </p>
      </td>
        ...
        {order?.items?.length > 0 ? (
          <div className="flex flex-col gap-1 text-xs sm:text-sm">
            {order.items.slice(0, 2).map((item, i) => (
              <p key={i} className="text-gray-600 truncate max-w-[180px]" title={`${item.productName} × ${item.quantity} — ₹${item.subtotal}`}>
